...
(function(){
	...
	let yScale = d3.scaleLinear()
		.range([0, 595])
		.domain([0, 744901*1.01]);  //<--THIS NEEDS TO BE DYNAMIC!!!
...
//Begin script when window loads
	...
	function setMap() {
		...
		Promise.all(promises).then(function (values) {
			...
			let stateLabels = map.selectAll(".stateLabels")
				.append("Text")
	...
	function makeYScale(data){
		...
		yScale =  d3.scaleLinear().range([0, 595]).domain([0, d3.max(domainArray)*1.01]);
		return yScale;
	};

		//Function to make yAxis
	function makeYAxis(data){
		...
		yAxis =  d3.axisLeft()
			.scale(d3.scaleLinear().range([590, 0]).domain([0, d3.max(domainArray)*1.01]));
		return yAxis;
	};
	...
	function setChart(zillowData, colorScale){
		...
		let yAxis = makeYAxis(zillowData);
		//Place axis
	...
	function changeAttribute(attribute, zillowData){
		...
		let yAxis = makeYAxis(zillowData);
		//Recreate the yScale
		let yScale = makeYScale(zillowData);
		...
		let chartTitle = d3.select(".chartTitle")
			.text(function(d){
				if ([expressed] == "2018_ZHVI_ALL"){
					return "Average ZHVI (All Homes)";
				} if ([expressed] == "2018_ZHVI_SFR"){
					return "Average ZHVI (Single Family Residences)";
				} if ([expressed] == "2018_ZHVI_CONDO"){
					return "Average ZHVI (Condominiums)";
				} if ([expressed] == "2018_MED_VAL_SF"){
					return "Median Home Value per Square Foot";
				} if ([expressed] == "2018_PCT_HOMES_INC_VAL"){
					return "Percent of Homes Increased in Value";
				} if ([expressed] == "2018_PCT_HOMES_DEC_VAL"){
					return "Percent of Homes Decreased in Value";
				};
			});
	}
	...
	function setLabel(props){
		let value = props[expressed];
		//Label content
		let labelAttribute = "<h1>" + value +
			"</h1><b>" + expressed + "</b>";
